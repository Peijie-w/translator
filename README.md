# Ubersetzer

一个可直接加载到 Chrome / Edge 的浏览器扩展原型：

- 普通网页中，鼠标长时间悬停在单词上会显示翻译结果
- 通过扩展内置的 PDF Viewer 打开 PDF 后，也可以对 PDF 文本执行同样的悬停翻译
- 目标语言、源语言、悬停延迟、PDF 默认缩放都可以在设置页调整
- 支持历史记录和收藏词本，翻译后可以点击 `Save` 保存到 Notebook

## 当前能力

- 普通网页：支持
- 文本型 PDF：支持
- 浏览器原生 PDF 标签页：由于浏览器权限限制，不能直接注入悬停层，所以需要用扩展自带 Viewer 打开
- 扫描版 / 图片型 PDF OCR：这一版还没有接入 OCR

## 安装与构建

```bash
npm install
npm run build
```

构建完成后，产物在 `dist/`：

1. 打开 Chrome 或 Edge 的扩展管理页
2. 开启“开发者模式”
3. 选择“加载已解压的扩展程序”
4. 选中当前项目下的 `dist` 目录

## Safari 本地测试

Safari 不能直接加载 `dist/` 目录，需要先转换成 Safari Web Extension 的 Xcode 工程：

```bash
npm run build
xcrun safari-web-extension-converter dist \
  --project-location safari \
  --app-name Translator \
  --bundle-identifier com.peijiew.translator \
  --macos-only \
  --copy-resources \
  --no-open \
  --no-prompt \
  --force
open safari/Translator/Translator.xcodeproj
```

然后在 Xcode 里：

1. 选择 `Translator` 这个 macOS App target
2. 点击运行按钮
3. App 启动后打开 Safari
4. 进入 `Safari > 设置 > 扩展`
5. 勾选 `Translator Extension`

如果 Safari 看不到扩展，先打开 Safari 的开发菜单，再启用“允许未签名扩展”。不同 Safari 版本菜单位置可能略有不同，通常在 `Safari > 设置 > 高级` 里开启开发菜单。

## 使用方式

### 网页悬停翻译

打开任意网页，把鼠标停留在一个词上，等待默认约 700ms，就会出现翻译浮层。

如果需要翻译一整句或一整段，用鼠标拖选那段文字，松开鼠标后会翻译整段选中文本。

如果你想把这个词或短语记下来，可以点击翻译浮层里的 `Save` 按钮，把它加入 Notebook。

### PDF 翻译

有两种方式：

1. 打开一个 PDF 链接后，点击扩展弹窗里的 `Open current PDF in viewer`
2. 在 PDF 链接上右键，选择 `Open linked PDF in Ubersetzer`

如果是本地 PDF，也可以直接打开扩展自带 Viewer，然后通过文件选择器导入。

在 PDF Viewer 里也可以拖选一段文本，松开鼠标后会翻译整段选中文本。

PDF Viewer 的翻译浮层同样支持 `Save`，可以把单词或整段收藏进 Notebook。

### Notebook

扩展会自动保留翻译历史，并且你可以手动收藏重点词条：

- 所有翻译记录都会进入历史
- 点击 `Save` 会把当前词条标记为收藏
- 在扩展弹窗里点击 `Open notebook`，或者直接打开设置页，就可以查看、搜索、取消收藏或删除记录

## 翻译服务

当前版本默认通过 Google 的网页翻译接口获取结果，适合做原型验证。若你要长期使用，建议下一步切换为你自己的正式翻译 API。

# translator
